    ...
    if ($ensure == 'present') {
      if ($kernel_ver =~ /^6\.2|^6\.3/) {
        if ($installdns == 'yes'){
          ...
          exec { 'Config ADDS':
            command     => "Start-Transcript C:\logicworks\ad-log.txt; Import-Module ADDSDeployment; Start-Sleep -s 300; Install-ADDSForest -Force -DomainName ${domainname} -DomainMode ${domainlevel} -DomainNetbiosName ${netbiosdomainname} -ForestMode ${forestlevel} -DatabasePath ${databasepath} -LogPath ${logpath} -SysvolPath ${sysvolpath} -SafeModeAdministratorPassword (convertto-securestring '${dsrmpassword}' -asplaintext -force) -InstallDns",
            provider    => powershell,
